  ...
  try {
    ...
    if (Password.length < 5 && Password.length > 10) {
      return res
        .status(400)
        .send({
          message: "please provide Password between 5 and 10 charrecter",
        });
    }
    ...
    if (!isValid(PositionInTheCompany)) {
      return res
        .status(400)
        .send({ message: "please provide PositionInTheCompany" });
    }
    if (!isValid(Address)) {
    ...
    return res
      .status(200)
      .send({ message: "admin created successfully", data: createUser });
  } catch (err) {
    return res
      .status(500)
...
const applyLeave = async function (req, res) {
  try {
    ...
    if (TotalLeave && TotalLeave <= 0) {
      return res
        .status(400)
        .send({ message: "TotalLeave should be a positive number" });
    }
    ...
    const leaveDate = new Date(
      findAdminId.LeaveDate.toISOString().split("T")[0]
    );
  ...
  try {
    ...
    bycrypt.compare(Password, findEmail.Password, function (err, result) {
      if (result) {
        ...
        return res
          .status(200)
          .send({ message: "login successfully", token: data });
      } else {
...
const deleteAdmin = async function (req, res) {
  try {
    ...
    const findAdminId = await admin.findOne({
      _id: adminId,
    ...
    if (!findAdminId) {
      return res
        .status(400)
        .send({ message: "adminId not found or already deleted" });
    }
    ...
    return res
      .status(200)
      .send({ message: "admin deleted successfully", data: deleteAdmin });
  } catch (err) {
    return res
      .status(500)
      .send({ message: "something went wrong", err: err.message });
...
const getLeaveByAdmin = async function (req, res) {
  try {
    const { Leave, name } = req.query;

    if (!Leave && !name) {
      return res.status(400).send({ message: "Please provide Leave or name" });
    }

    const leave = ["sickLeave", "earnedLeave", "CompansatoryLeave"];
    if (Leave && !leave.includes(Leave)) {
      return res.status(400).send({ message: "Please provide a valid Leave" });
    }
    ...
    const getLeave = await admin.find(query);
